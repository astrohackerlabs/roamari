...
/// Hard refresh: Shift+R (Super optional). Control|Browse when events reach roamari.
...
#[command(
    name = "roamari",
    about = "Roamari — open URLs in Terminal browser panes",
...
fn main() -> io::Result<()> {
    if std::env::args().skip(1).any(is_version_arg) {
        println!("Roamari {}", env!("ASTROHACKER_CLI_VERSION"));
        return Ok(());
    }
...
/// it can restore the cursor. That races roamari's background `event::read`
...
///
/// Display-only: the path basename (last `/` segment). Selector `chromium`
/// shows `chromium`; helper `roamari-chromiumd` shows `roamari-chromiumd`.
fn browser_display_label(browser: &str) -> &str {
    browser.rsplit('/').next().unwrap_or(browser)
}
...
mod tests {
    ...
    fn loading_screen_uses_browser_label_and_immediate_warning() {
        ...
        let helper = render_loading_probe(browser_display_label("roamari-chromiumd"));
        assert!(
            helper.capture.contains("Waiting for roamari-chromiumd"),
            "loading should show full helper basename\n{}",
            helper.capture
        );
        ...
        assert!(
            helper_path
                .capture
                .contains("Waiting for roamari-chromiumd"),
            "loading should show basename from a helper path\n{}",
            helper_path.capture
        );
        let selector = render_loading_probe(browser_display_label("chromium"));
        assert!(
            selector.capture.contains("Waiting for chromium"),
            "selector chromium still displays as its basename\n{}",
            selector.capture
        );
        ...
        assert!(
            ...
            "loading should show full basename for vendor-foobard\n{}",
            third_party.capture
        );
        assert!(
            !third_party.capture.contains("Waiting for foobar\n")
                && !third_party.capture.contains("Waiting for foobar "),
            "loading must not strip {{prefix}}-{{stem}}d to {{stem}}\n{}",
            third_party.capture
        );
        assert!(
            helper
    ...
    #[test]
    fn browser_display_label_uses_basename_only() {
        for (input, expected) in [
            ...
            ("/opt/homebrew/bin/ah-chromiumd", "ah-chromiumd"),
            ("vendor-foobard", "vendor-foobard"),
            ("chromium", "chromium"),
            ("webkit", "webkit"),
            ("custom-engine", "custom-engine"),
            ("/tmp/custom-engine", "custom-engine"),
            ("somethingd", "somethingd"),
        ] {
